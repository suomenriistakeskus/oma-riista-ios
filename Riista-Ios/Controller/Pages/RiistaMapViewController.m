...
float const DEFAULT_ZOOM_TO_LEVEL = 15.0f;
...
@implementation RiistaMapViewController
{
    ...
    /**
     ...
     TODO: check usages after iOS10 support has been dropped. The same functionality (fullscreen) can be achieved on >= iOS11 with storyboard changes.
        -> Add extend edges under bottom bars and opaque bars to both Main.storyboard and Details.storyboard maps.
     */
...
{
    BOOL hide = [RiistaSettings hideMapButtons];

    [self animateButtonToolbar:hide];
}

- (void)updateLocation:(CLLocation*)newLocation animateCamera:(BOOL)animateCamera
{
    if (animateCamera) {
        [self animateAndZoomCameraTo:newLocation zoom:DEFAULT_ZOOM_TO_LEVEL];
    }
    else {
        [_mapView moveCamera:[GMSCameraUpdate setTarget:newLocation.coordinate zoom:DEFAULT_ZOOM_TO_LEVEL]];
    }
...
{
    if (_gpsLocation != nil) {
        [self animateAndZoomCameraTo:_gpsLocation zoom:DEFAULT_ZOOM_TO_LEVEL];
    }
}
...
{
    RiistaMapSettingsViewController* controller = [self.storyboard instantiateViewControllerWithIdentifier:@"mapSettingsController"];
    controller.areaManager = areaManager;
    ...
    if ([marker.userData conformsToProtocol:@protocol(GMUClusterItem)]) {
        ...
        switch (mapMarker.type) {
            case RiistaEntryTypeHarvest:
                ...
                marker.iconView = [[UIImageView alloc] initWithImage:image highlightedImage:nil];
                break;
            case RiistaEntryTypeObservation:
...
- (BOOL)clusterManager:(GMUClusterManager *)clusterManager didTapCluster:(id<GMUCluster>)cluster {
    if (_mapView.camera.zoom == MAX_ZOOM) {
