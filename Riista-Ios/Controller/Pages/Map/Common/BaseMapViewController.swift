...
protocol CanCenterMapAtLocation: AnyObject {
    func centerMapAtLocation(location: CLLocationCoordinate2D, zoomLevel: Float)
}
...
class BaseMapViewController: UIViewController, GMSMapViewDelegate, CanCenterMapAtLocation {
    ...
    func initializeCamera() {
        ...
        mapView.camera = getInitialCameraPosition()
    }

    func getInitialCameraPosition() -> GMSCameraPosition {
    ...
    func centerMapAtLocation(location: CLLocationCoordinate2D, zoomLevel: Float) {
        mapView.moveCamera(GMSCameraUpdate.setTarget(location, zoom: zoomLevel))
    }
}
